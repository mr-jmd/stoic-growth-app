...
**All 8 sprints (0–7) are complete; the app covers the full MVP loop** (onboarding → habits/streaks/relapse → morning/evening journal → crisis → virtue dashboard), 100% offline, no account. **Voice dictation remains the one deferred Sprint 5 carry-over** (needs native `speech_to_text`/`permission_handler` + physical-device verification) — see `docs/SPRINT_PLAN.md`.
