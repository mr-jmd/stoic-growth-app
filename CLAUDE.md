...
`README.md` (Spanish) is the **product proposal / vision**. The **MVP (Fase 1) is fully built** (all 8 sprints + voice). Current documentation:
- `docs/ARCHITECTURE.md` — the technical architecture in use (stack, layers, data, invariants, testing).
- `docs/features/` — one doc per MVP functionality (onboarding, habits, streaks/relapse, journal+voice, crisis, dashboard).
- `docs/SPRINT_PLAN.md` — historical record of the 8-sprint build (now complete), plus the project invariants (no paywalled streaks, no ads, no aggressive push, 100% offline, no required account), package-version rationale, and Fase 2 seams.

**The visual design system is being replanned from scratch.** The design brief was removed and the debug design gallery was deleted from the app. The `lib/src/core/design_system/` code is **interim** — it still works, but don't treat its choices as fixed rules, and don't add new design rules to the docs. Below, the per-sprint entries are kept as a build log.
...
**Sprint 1 (theming + i18n) is complete** — but the visual system is **interim and being replanned**, so this is described only structurally (no design rules here):
- **Theming** under `lib/src/core/design_system/` — a token layer (`StoicTokens`, a `ThemeExtension`) plus base components read through `context.stoic`. Feature code reads everything through the theme rather than hardcoding values, so the visual language can be replaced without touching features. Treat the specific current choices as provisional.
- **Fonts:** two OFL families vendored as variable fonts in `app/assets/fonts/`, declared in `pubspec.yaml`. **Never** add `google_fonts` (runtime fetch breaks the offline invariant).
...
- **Access:** `AppScaffold` already had a `crisisAccess` slot — home fills it with `CrisisAccessButton` → `/crisis` (one tap). `/crisis` is **exempt from the onboarding-gate redirect** so distress is never blocked by setup. Assets are declared under a new `assets:` block in `pubspec.yaml` (`assets/content/`).
...
The repo name `stoic-growth-app` is a technical placeholder. **"Andamio" is the working name** used in development; "Virtude" is the backup alternative if Andamio fails domain/social validation. The final brand name isn't locked yet (see README section 12).
